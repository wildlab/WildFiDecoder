import decoder.*;
import decoder.entries.MetaDataEntry;
...
public class Main {
    public static String SOFTWARE_VERSION ="8.0";

    enum SelectedMode {
        DecodeBinaryDataReceivedByPhoneOrGateway,
        DecodeProxLoggerBinaryData,
        DecodeHdLoggerBinaryData,
        DecodeGatewayMetadata,
        MergeAllBinaryFilesInCurrentDirectory,
        MergeAllBinaryFilesInSubDirectories,
        MergeAllBinaryFilesInCurrentDirectoryIntoTagSeperatedFiles,
        MergeAllBinaryFilesInSubSubDirectories,
        MergeCsvFilesInCurrentDirectory,
        MovebankUploadOfCsvFiles,
        ModeNotExisting,
        WrongInputFormat,
        ExitDexoder;
    }
    public static SelectedMode modeSelect() {
        ...
        {
            return SelectedMode.WrongInputFormat;
        }

        if(i == 0) return SelectedMode.DecodeBinaryDataReceivedByPhoneOrGateway;
        if(i == 1) return SelectedMode.DecodeProxLoggerBinaryData;
        if(i == 2) return SelectedMode.DecodeHdLoggerBinaryData;
        if(i == 3) return SelectedMode.DecodeGatewayMetadata;
        if(i == 4) return SelectedMode.MergeAllBinaryFilesInCurrentDirectory;
        if(i == 5) return SelectedMode.MergeAllBinaryFilesInSubDirectories;
        if(i == 6) return SelectedMode.MergeAllBinaryFilesInCurrentDirectoryIntoTagSeperatedFiles;
        if(i == 7) return SelectedMode.MergeAllBinaryFilesInSubSubDirectories;
        if(i == 8) return SelectedMode.MergeCsvFilesInCurrentDirectory;
        if(i == 9) return SelectedMode.MovebankUploadOfCsvFiles;
        if(i == 99) return SelectedMode.ExitDexoder;
        else return SelectedMode.ModeNotExisting;
    ...
    static void readConfigFile(IMUSettings imuSettings) {
        ...
        String ParentDirectoryOfConfigFile;
        try {
            ParentDirectoryOfConfigFile = new File(Main.class.getProtectionDomain().getCodeSource().getLocation()
                    .toURI()).getParent();
        ...
        try {
            br = new BufferedReader(new FileReader(ParentDirectoryOfConfigFile + File.separator + configFileName));
    ...
    public static void DecodeBinaryData(SelectedMode selectedMode, IMUSettings imuSettings, boolean useBurstForm)  throws IOException {
        ...
        boolean isMoveData = false;

        if (selectedMode == SelectedMode.DecodeProxLoggerBinaryData)
        {
            imuSettings.customPrefixLength = 2;
            isMoveData = false;
        }
        else if (selectedMode == SelectedMode.DecodeHdLoggerBinaryData)
        {
            imuSettings.customPrefixLength = 0;
            isMoveData = true;
        }

        String selectedFileToConvert = fileSelect(".bin");
        if(selectedFileToConvert.isEmpty())
            return;
        ...
        if (selectedMode == SelectedMode.DecodeProxLoggerBinaryData || selectedMode == SelectedMode.DecodeHdLoggerBinaryData)
        {
            ...
            if (!decodingManager.ConvertBytesToMessages(fileData, tagNames, true, imuSettings))
            {
                Log.d("Error", "Could not load file!");
                return;
            }
            ...
            CountDownLatch countDownLatch = new CountDownLatch(tagNames.size());
            ArrayList<ProgressBar> decodeProgressBars = new ArrayList<>();
            for (String tagIterator : tagNames)
            {
                ...
                decodingManager.DecodeDataInSeperateThread(tagIterator,fileNameCsvComplete, imuSettings, useBurstForm, isMoveData , imuFrequency, countDownLatch);
        ...
        if (selectedMode == SelectedMode.DecodeBinaryDataReceivedByPhoneOrGateway)
        {
        ...
        Log.dNoLog("main","Decoding took: " + estimatedTime + "ms");
    }
    ...
    public static void main(String[] args)  throws IOException {
        ...
        while(true) {
            ...
            SelectedMode selectedMode = modeSelect();

            switch (selectedMode)
            {
                //todo handle binaryPhoneThing

                case DecodeProxLoggerBinaryData:
                    useBurstForm = burstFormSelect();
                    DecodeBinaryData(selectedMode, imuSettings, useBurstForm);
                    break;

                case DecodeHdLoggerBinaryData:
                    useBurstForm = burstFormSelect();
                    DecodeBinaryData(selectedMode, imuSettings, useBurstForm);
                    break;

                case DecodeGatewayMetadata:
                ...
                case MergeAllBinaryFilesInCurrentDirectory:
                    mergeBinaryFiles(".bin");
                    break;

                case MergeAllBinaryFilesInSubDirectories:
                    mergeBinaryFilesInSubFolders(".bin");
                    break;

                case MergeAllBinaryFilesInCurrentDirectoryIntoTagSeperatedFiles:
                    mergeBinaryFilesInSubFoldersHdLoggerSplittedMacs(".bin");
                    break;

                case MergeAllBinaryFilesInSubSubDirectories:
                    mergeBinaryFilesInSubSubFolders(".bin");
                    break;

                case MergeCsvFilesInCurrentDirectory:
                    try {
                        mergeCSVFiles();
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                ...
                case ModeNotExisting:
                    Log.dNoLog("Error", "Invalid Selection! Assuming you want to decode data.");
                    break;
