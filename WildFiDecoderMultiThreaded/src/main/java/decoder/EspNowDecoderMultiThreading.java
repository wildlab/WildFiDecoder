...
import Logging.Log;
import decoder.entries.LogEntry;
...
public class EspNowDecoderMultiThreading extends Thread
{
    private int LINE_BUFFER_SIZE = 2000;
    public ArrayList<EspNowMessage> messagesSingleTag = new ArrayList<>();
    ...
    boolean useBurstForm;

    int imuFrequency;

    CountDownLatch countDownLatch;

    public EspNowDecoderMultiThreading() { }
    public EspNowDecoderMultiThreading(ArrayList<EspNowMessage> espNowMessagesSingleTag, String tagId, String filename, IMUSettings imusettings, boolean burstForm, boolean isMoveData, int imuFreq, CountDownLatch cntDownLatch)
    {
        messagesSingleTag = espNowMessagesSingleTag;
        selectedTagName = tagId;
        fileName = filename;
        imuSettings = imusettings;
        useBurstForm = burstForm;

        if(isMoveData)
            LINE_BUFFER_SIZE = 7000;

        imuFrequency = imuFreq;
        countDownLatch = cntDownLatch;
    ...
    public boolean stepFiveWriteResultToFile(String fileNameCsvComplete, long selectedStartTimestamp, long selectedEndTimestamp, IMUSettings imuSettings, int outputModulo, boolean useBurstForm, int imuFrequency, ProgressBar pb) throws IOException {
        ...
        while (iterator < stream.length) {
            ...
            if ((line.length() == LogEntry.INDEX(LINE_BUFFER_SIZE)) || (iterator == stream.length)) {
                ...
                if(!entireLineError) {
                    ...
                    e.decode(name, line, false, imuSettings, imuFrequency);
    ...
    public boolean stepFourSelectedEspNowMessagesToDataStream() {
        ...
        boolean first = true;
        for (EspNowMessage e : messagesSingleTag) {
            if (e.isDataMessage) {
                if (first) {
                    first = false;
    ...
    public boolean stepThreeCheckIfPointersAreConsecutive() {
        ...
        lastLength = temp3;
        messagesSingleTag.removeAll(messagesToRemove);
    ...
    public ArrayList<EspNowMessage> stepTwoReduceDataToOneTag(String selectedTagName,ArrayList<EspNowMessage> espNowMessagesAll ) {
        int cntAdded = 0;
        ArrayList<EspNowMessage> espNowMessagesSingleTag = new ArrayList<>();
        ...
        return espNowMessagesSingleTag;
    }
    public boolean stepOneBytesToEspNowMessages(byte[] fileData, ArrayList<String> macs, ArrayList<EspNowMessage> espNowMessagesAll, boolean debug, IMUSettings imuSettings) {
    ...
    public void run() {
        //System.out.println("Task" + this.getName() + ":  TagID: " + selectedTagName + " decoding " + messagesSingleTag.size() + " messages");
        //try(ProgressBar pb = new ProgressBar(selectedTagName, (int)messagesSingleTag.size()/3))
        try(ProgressBar pb = new ProgressBarBuilder()
                .setStyle(ProgressBarStyle.ASCII)
                .setTaskName(selectedTagName)
                .setInitialMax((int)messagesSingleTag.size()/3)
                .setUnit(" kB",1024)
                .showSpeed()
                .build())
        {
            if (!stepThreeCheckIfPointersAreConsecutive())
            {
                ///Log.d("Error", "Esp now data messages are not consecutive (maybe due to some re-transmission)");
            }

            if (!stepFourSelectedEspNowMessagesToDataStream()) {
                ...
                try {
                    success = stepFiveWriteResultToFile(fileName, selectedStartTime, selectedEndTime, imuSettings, 10, useBurstForm, imuFrequency, pb);
                }
                catch (IOException ex)
                {
                    System.out.println (ex.toString());
                    System.out.println("Error occured while handling file");
                }
                //writerComplete.close();"
        ...
        countDownLatch.countDown(); //every thread need to be finished until main thread can continue
    }
}
